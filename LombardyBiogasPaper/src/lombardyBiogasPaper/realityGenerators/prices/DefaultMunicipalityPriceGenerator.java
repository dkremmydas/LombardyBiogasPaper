 ...
 *
 */
public class DefaultMunicipalityPriceGenerator implements PriceGenerator {

	private Municipality parentMunicipality;
	
	private double sigma ; //it is initialized to 0.3f in the constructor
	
	public DefaultMunicipalityPriceGenerator(Municipality m) {
		this(m,0.05d);
	}
	...
	public HashMap<ArableCrop,Long> getPrices() {
		...
		for(ArableCrop c: cs) {
			...
			Long oldVal = SimulationContext.getInstance()
					.getRealityGenerator()
					.getMunicipalityPriceHistory(this.parentMunicipality)
					.get(currentYear-1,c);
